"""
Phase 4 - Texas healthcare demand campaign: generate the assembled email body.

SEPARATE from generate_healthcare_demand.py on purpose. That script feeds the
LIVE Indiana campaign (f59f4fa5, "Brent - B3 Resource solutions") and must not
be disturbed. This one has different copy, a different variable set (5 slots,
no persona / age-band routing) and targets a different tab.

Copy is Jude's, approved 2026-07-22. FIXED except the marked slots. Do not
reword the identity line, the proof line, the contingent clause or the CTA.

  THE PROOF LINE IS A FACTUAL CLAIM ABOUT A REAL PLACEMENT (an aesthetics
  clinic in Texas, NP role, 45 days open, filled in a week, via a recruiter
  in Jude's network who is NOT Brent). "a clinic" is HARD-CODED. It must
  never be varied to mirror the recipient's employer type -- that would
  fabricate the one piece of credibility the whole email rests on.
...
Per lead, one GPT-4.1 JSON call extracts 4 slots from the job description:
  ...
  employer_type   plural employer noun describing THIS employer
  city            the city the ROLE is in, per the POSTING (the sheet's City
                  column is wrong on at least one row: S2S is listed as
                  Flower Mound but the posting is for Frisco)

Everything is validated with deterministic fallbacks -- the email ships with a
generic-but-true slot rather than an LLM guess.

Writes: Z body, AE-AJ audit trail.
  NOTE AD is "Keep Reason" on this sheet (the DM-title adjudication pass) and
  is NEVER written. The audit block starts at AE.

Batch-of-10 writes; idempotent (skips rows where AE is already set).

Usage:
  python3 -W ignore generate_texas_demand.py --sheet_url "URL" --preview 20
  python3 -W ignore generate_texas_demand.py --sheet_url "URL" --limit 25
  python3 -W ignore generate_texas_demand.py --sheet_url "URL"
"""
...
AUDIT_HDRS = ["cleaned_role", "role_plural", "employer_type",
              "casual_first", "city_used", "gen_status"]
...
My name is Rood. I connect Texas healthcare employers with recruiters that can actually deliver.

Recently a clinic here in Texas had an NP role sitting for 45 days. I introduced them to one of the recruiters in my network and it was filled in a week.

I know a recruiter who helps {employer_type} find pre-vetted {role_plural} in Texas. He only works on contingent, so nothing is owed unless the hire sticks.

Before I make an intro, I just wanted to check timing. Is this hire a priority right now or more exploratory?"""
...
REQUIRED = [
    ...
    "Is this hire a priority right now or more exploratory?",
]
...
{"cleaned_role": "...", "role_plural": "...", "employer_type": "...", "city": "..."}
...
def build_body(v):
    body = BODY.format(first=v["casual_first"], cleaned_role=v["cleaned_role"],
                       city=v["city"], employer_type=v["employer_type"],
