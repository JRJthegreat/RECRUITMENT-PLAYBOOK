"""
Shared schema and utilities for healthcare-linkedin-leads.
...
Field mapping based on insight_api_labs~linkedin-jobs-scraper output:
  ...
  companyUrl      → Company Website (LinkedIn company URL — enriched later)
  (no size field) → Company Size    (blank — filled by enrich_company_profiles.py)
  benefits list   → Benefits
  location string → City, State     (parsed)
  jobUrl          → LinkedIn Job URL (column AC)

Schema is the 29-col layout shared with hr-leads-indeed downstream scripts, with
two differences from the Indeed pipeline:
  AB: Role Type        (was "Seniority" in hr-linkedin — clinical role of the posting)
  AC: LinkedIn Job URL (instead of Indeed URL)
"""
...
HEADERS = [
    ...
    "Company Website",     # L — companyUrl (LinkedIn company page; enriched later)
    "Company Size",        # M — blank at ingest, filled by enrich_company_profiles.py
    "Revenue",             # N — blank
    "CEO Name",            # O — blank
    "Company Description", # P — blank, filled by enrich_company_profiles.py
    ...
    # Derived
    "Role Type",           # AB — clinical role of the posting
    "LinkedIn Job URL",    # AC — jobUrl
]


def classify_role(job_title):
    """Classify the clinical role of the posting. NP-only for now; Physician/PA
    recognised so the schema is future-proof if keywords expand."""
    t = (job_title or "").lower().strip()
    if not t:
        return ""
    if any(p in t for p in ("nurse practitioner", "fnp", "aprn", "np-c", "family np", "nurse pract")):
        return "Nurse Practitioner"
    if any(p in t for p in ("physician assistant", "pa-c")):
        return "PA"
    if "physician" in t or "family medicine" in t or "family practice" in t:
...
def map_to_row(item):
    ...
    return [
        ...
        item.get("companyUrl", ""),
        "",  # Company Size — not available from LinkedIn jobs actor
        "",  # Revenue
        "",  # CEO Name
        "",  # Company Description
        benefits_str,
