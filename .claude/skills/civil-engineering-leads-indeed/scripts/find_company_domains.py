...
Needed before Phase 2 (find_dm.py) because the old DM lookup search used only
the company name, which collides on short/generic names (e.g. "Legrand" vs
"Legrand Care"). With a domain anchor, the DM search can verify the person
actually works at the target company.

Output: col L (Company Website) populated with bare registered domain
(e.g. "legrandcare.com"). Skips rows that already have a valid-looking
domain in col L.
...
BLOCKED_HOSTS = {
    ...
    "monster.co.uk", "jobserve.com", "jobs.ac.uk", "jobtoday.com",
    ...
    "reddit.com", "quora.com", "amazon.co.uk", "amazon.com",
    "bbc.co.uk", "theguardian.com", "thetimes.co.uk",
    # UK civil-eng professional bodies & trade press
    "cibse.org", "cibsejournal.com", "ice.org.uk", "istructe.org",
    "iet.org", "theiet.org", "rics.org", "ciob.org", "rtpi.org",
    "ioshworld.com", "newcivilengineer.com", "constructionnews.co.uk",
    "building.co.uk", "theconstructionindex.co.uk",
    "contractjournal.com", "planningresource.co.uk",
}
...
def is_public_sector_host(host):
    """Block any *.gov.uk, *.ac.uk, *.nhs.uk, *.police.uk — and also the bare
    TLDs (e.g. 'gov.uk' after www-stripping), which are never a company site."""
...
STOP_TOKENS = {
    ...
    "consultancy", "engineering", "construction", "contracting",
    "contractors", "partnership", "partners", "solutions", "global",
}
...
def registered_domain(host):
    ...
    if len(parts) <= 2:
        return host
    # UK two-part TLDs
    if parts[-2] in ("co", "org", "ac", "gov", "ltd", "plc", "me") and parts[-1] == "uk":
        return ".".join(parts[-3:])
    return ".".join(parts[-2:])
...
LLM_SYSTEM = (
    ...
    "(e.g. 'example.co.uk') or the word NONE. "
    "Rules:\n"
...
def build_query(company):
    """Strip trailing legal suffixes (Ltd/Limited/PLC/…) before searching.
    'WB Power Services Limited' triggers Companies House / council docs;
    'WB Power Services' surfaces the actual website."""
    cleaned = LEGAL_SUFFIX_RE.sub("", company.strip()).strip()
    return f'"{cleaned}" UK official website'
...
def main():
    ...
    for b in range(num_batches):
        ...
        for q, comp in q_to_company.items():
            ...
            # Always write — empty string clears any stale value in col L.
