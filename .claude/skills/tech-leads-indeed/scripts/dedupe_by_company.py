...
Rules:
  ...
  2. Winner = highest seniority tier (CTO/VP > Director/Head > Principal/Staff >
     Lead/Architect > Senior > Mid > Junior).
...
def classify_tier(title):
    t = (title or "").lower()
    # C-level / VP — hiring at the top signals a broader mandate
    if re.search(r"\b(cto|chief\s+technology|chief\s+technical)\b", t):
        return 9
    if re.search(r"\bvp\s+(of\s+)?engineering\b|\bvice\s+president\s+(of\s+)?engineering\b", t):
        return 9
    # Director / Head of — owns budget + hiring plan
    if re.search(r"\b(director|head\s+of)\b", t):
        return 8
    # Principal / Staff — IC leadership
    if re.search(r"\b(principal|staff)\b", t):
        return 7
    # Lead / Architect / Tech Lead
    if re.search(r"\b(lead|architect|tech\s+lead)\b", t):
        return 6
    # Senior / Sr.
    if re.search(r"\b(senior|snr|sr\.?)\b", t):
        return 5
    # Junior / Graduate / Intern / Trainee
    if re.search(r"\b(junior|jnr|jr\.?|graduate|grad|trainee|intern|apprentice)\b", t):
        return 1
    # Default mid-level engineer / developer
    return 3


TIER_LABEL = {
    9: "CTO/VP Eng", 8: "Director/Head", 7: "Principal/Staff",
    6: "Lead/Architect", 5: "Senior", 3: "Mid-level", 1: "Junior",
}
...
COMPANY_SUFFIX_PATTERNS = [
    ...
    # European legal suffixes
    r"\bgmbh\b", r"\bag\b", r"\bsas\b", r"\bsa\b", r"\bsarl\b",
    r"\bb\.?v\.?\b", r"\bn\.?v\.?\b", r"\bs\.?r\.?l\.?\b", r"\bs\.?p\.?a\.?\b",
    r"\ba/s\b", r"\bab\b", r"\boy\b", r"\boyj\b",
    # Country tags
    r"\(uk\)", r"\buk\b", r"\beu\b", r"\(eu\)",
]
...
def main():
    parser = argparse.ArgumentParser(description="Dedup tech leads by company")
    ...
    for t in sorted(tier_counts.keys(), reverse=True):
        print(f"  Tier {t} ({TIER_LABEL.get(t, '?'):18s}): {tier_counts[t]}")
