"""
Phase 2 (demand side): Find decision makers for small healthcare practices
via Google Search + LinkedIn.

This is the 29-col DEMAND-side finder (company=K, DM=T/U/V). Do NOT confuse it
with find_dm.py in this skill, which is the repurposed staffing-AGENCY version
(company=C, DM=E/P/Q/R) and reads the wrong columns on this schema.

DM rule (the decision to engage an outside recruiter sits with whoever owns the
P&L — for owner-operated practices that's the owner regardless of location count):
  - 51-200 employees → practice has a real HR function: target HR Manager/Director
    first, then fall back to Owner/Medical Director.
  - blank / <50 employees → no HR dept: target Practice/Office/Clinic Manager
    first, then fall back to Owner / Medical Director / CEO.
  - col O (CEO/owner name from Indeed) already present → written directly to col T,
    no search spent (free head start; the owner is a valid DM at any size).

Search is deduped by company: one lookup per unique company name, then the
result is fanned out to every row of that company (handles multi-location
practices without deleting rows — no dedupe required first).

Two passes per company max:
  Pass 1 = primary target from determine_target().
  Pass 2 = next_tier() fallback.
After Pass 2 misses, the row is left empty for Phase 3 (AMF /decision-maker).

Tiers:
  practice_manager — Practice/Office/Clinic Manager or Practice Administrator.
  hr               — HR Manager/Director / Human Resources / People Operations.
  owner            — Owner / Medical Director / CEO / President / Managing Partner.
"""
...
# Headcount band above which a practice typically has a dedicated HR function.
HR_FUNCTION_MIN = 51
...
def determine_target(job_title, employee_count_str):
    """Pick the primary DM tier from headcount. Returns (target, reasoning).
    Targets: 'practice_manager', 'hr', 'owner'.

    Routing is by company size, not the clinical role being hired (the role is
    always NP/PA/physician — never the DM):
      - 51-200 employees → practice has a real HR function → target HR first.
      - blank / <50      → no HR dept → target Practice/Office Manager first.
    """
    upper = parse_employee_count(employee_count_str)
    if upper is not None and upper >= HR_FUNCTION_MIN:
        return "hr", f"{upper}-employee practice has HR; target HR Manager/Director"
    return "practice_manager", "Small/unknown-size practice; target Practice/Office Manager first"


def next_tier(target):
    """Fallback chain: practice_manager → owner, hr → owner. Owner is terminal."""
    return {
        "practice_manager": "owner",
        "hr": "owner",
        "owner": "owner",
    }.get(target, "owner")
...
def build_search_query(company_name, target_level, target_domain=""):
    ...
    if target_level == "practice_manager":
        return (
            f'{co} '
            f'("Practice Manager" OR "Office Manager" OR "Clinic Manager" '
            f'OR "Practice Administrator" OR "Office Administrator") '
            f'site:linkedin.com/in/'
        )
    if target_level == "hr":
        return (
            f'{co} '
            f'("HR Manager" OR "HR Director" OR "Human Resources Manager" '
            f'OR "Director of Human Resources" OR "People Operations" OR "Head of People") '
            f'site:linkedin.com/in/'
        )
    if target_level == "owner":
        return (
            f'{co} '
            f'("Owner" OR "Medical Director" OR "CEO" OR "President" '
            f'OR "Founder" OR "Managing Partner") '
            f'site:linkedin.com/in/'
        )
...
def title_matches_target(title_text, target_level):
    ...
    if target_level == "practice_manager":
        # Manager/administrator of the practice/office/clinic. Require the
        # role noun so plain "Manager" of an unrelated dept doesn't pass.
        return any(kw in t for kw in (
            "practice manager", "office manager", "clinic manager",
            "practice administrator", "office administrator",
            "clinical operations manager", "operations manager",
        ))
    ...
    if target_level == "hr":
        ...
        starts_senior = t.startswith((
            "hr ", "human resources", "vp", "vice president", "svp",
    ...
    if target_level == "owner":
        starts = (
            "owner", "co-owner", "co owner", "founder", "co-founder",
            "cofounder", "co founder", "medical director", "ceo",
...
def process_batch(leads, target_map, dry_run):
    ...
    for q, lead in query_to_lead.items():
        ...
        if matched:
            sibs = lead.get("sibling_rows", [lead["sheet_row"]])
            for sr in sibs:
                updates.append({
                    "sheet_row": sr,
            ...
            fan = f" (+{len(sibs) - 1} locations)" if len(sibs) > 1 else ""
            log_lines.append(f"    Row {lead['sheet_row']}: {lead['company_name']} → "
                             f"{matched['name']} ({matched['title']}) [{target}]{fan}")
            found += 1
        else:
...
def prefill_ceo_from_col_o(service, sheet_id, tab_name, data_rows, dry_run):
    """Pre-pass: wherever col O already has a CEO/owner name and col T is empty,
    write that name straight to col T — no Apify search needed. The owner is a
    valid DM for these owner-operated practices at any size, and the title is
    set to 'Owner / CEO' so downstream email enrichment routes it correctly."""
    updates = []
    ...
    for u in updates[:10]:
        print(f"  row {u['sheet_row']}: {u['company']!r}  CEO={u['ceo_name']!r}")
    if len(updates) > 10:
    ...
    write_data = []
    for u in updates:
        write_data.append({"range": f"'{tab_name}'!{col_letter(COL_DM_NAME)}{u['sheet_row']}",
                           "values": [[u["ceo_name"]]]})
        write_data.append({"range": f"'{tab_name}'!{col_letter(COL_DM_TITLE)}{u['sheet_row']}",
                           "values": [["Owner / CEO"]]})
    CHUNK = 200
...
def collect_leads(rows, target_fn, limit):
    """One lead per unique company (deduped by normalized name). Each lead
    carries sibling_rows = every sheet row of that company still missing a DM,
    so a single search fans out to all of a multi-location practice's rows
    without requiring a prior dedupe pass."""
    leads = []
    target_map = {}
    by_company = {}  # norm name -> lead
    for i, row in enumerate(rows):
        ...
        if not company_name:
            continue
        key = " ".join(name_words(company_name)) or company_name.lower()
        sheet_row = i + 2
        if key in by_company:
            by_company[key]["sibling_rows"].append(sheet_row)
            continue
