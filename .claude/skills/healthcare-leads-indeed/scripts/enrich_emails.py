"""
Phase 3: Find DMs and/or emails via AnyMail Finder (Healthcare pipeline).
...
def _is_same_level_conflict(job_title, dm_title):
    """Always False in healthcare — practice manager is never the clinical role being hired."""
    return False
...
def find_dm_with_fallback(domain, company_name, *categories):
    last = None
    for cat in categories:
        r = find_dm_one_call(domain, company_name, cat)
...
def process_dm(lead):
    domain = extract_domain(lead["company_website"])
    categories = categories_for(lead["job_title"], lead["company_size"])
    result = find_dm_with_fallback(domain, lead["company_name"], *categories)
    return {**lead, **result, "categories": categories}
...
def main():
    parser = argparse.ArgumentParser(description="AMF: enrich emails (known DMs) + find DMs (missing rows) — healthcare")
    ...
    print("=== Enrich via AnyMail Finder (Healthcare) ===\n")
    ...
    print(f"  Total AMF calls ~= {len(person_leads) + 3 * len(dm_leads)} (DM mode uses 3 calls worst case)\n")
    ...
    if args.dry_run:
        ...
        for l in dm_leads[:10]:
            cats = categories_for(l["job_title"], l["company_size"])
            dom = extract_domain(l["company_website"])
            print(f"  B row {l['sheet_row']}: {l['company_name']!r} @ {dom or '?'} -> {'/'.join(cats)}")
        return
