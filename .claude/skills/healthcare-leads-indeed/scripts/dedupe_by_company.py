"""
Phase 1.8: Dedupe by company — keep one row per practice.

Rules:
  1. Group rows by normalized company name.
  2. Winner = highest role tier: Physician > NP > PA > unknown.
...
# --- Role tier (higher = more clinical priority = wins dedup) ---

def classify_tier(title):
    """Physician outranks NP outranks PA. Tie → unknown (mid)."""
    t = (title or "").lower()
    if any(p in t for p in ("physician", "family medicine", "family practice", "family doctor")):
        # Exclude NP/PA variants that contain 'physician' (e.g. 'physician assistant')
        if "assistant" not in t:
            return 9
    if any(p in t for p in ("nurse practitioner", "fnp", "aprn", "np-c", "family np")):
        return 7
    if any(p in t for p in ("physician assistant", "pa-c", " pa ", "pa,")):
        return 6
    return 3  # unknown / other clinical


TIER_LABEL = {
    9: "Physician", 7: "NP", 6: "PA", 3: "Other",
}
...
def main():
    parser = argparse.ArgumentParser(description="Dedup healthcare leads by practice/company")
    ...
    print(f"=== Dedup by Practice/Company ({mode}) ===")
    ...
    for key, items in groups.items():
        items.sort(key=lambda x: x["date"])          # oldest first (tiebreak)
    ...
    print(f"Unique practices: {len(groups)}")
    ...
    print("Winner role breakdown:")
    for t in sorted(tier_counts.keys(), reverse=True):
        print(f"  Tier {t} ({TIER_LABEL.get(t, '?'):12s}): {tier_counts[t]}")

    multi = sorted(
        [(k, v) for k, v in groups.items() if len(v) > 1],
        key=lambda kv: -len(kv[1]),
    )
    print(f"\nPractices with >1 job posting: {len(multi)}")
    print("Sample (top 10 by # of postings):")
    for key, items in multi[:10]:
        win = items[0]
        print(f"\n  {win['company_raw']!r}  ({len(items)} postings)")
        print(f"    WINNER   tier {win['tier']:2d} ({TIER_LABEL.get(win['tier'], '?')}) "
              f"{win['date']}  {win['title']!r}")
        for loser in items[1:]:
            print(f"    drop     tier {loser['tier']:2d} ({TIER_LABEL.get(loser['tier'], '?')}) "
