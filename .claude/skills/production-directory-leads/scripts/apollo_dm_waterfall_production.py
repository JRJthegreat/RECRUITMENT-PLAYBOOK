"""
Apollo DM Waterfall (PRODUCTION-HOUSE variant) — fallback pass for rows
find_dm_pm.py's Purple Magic lane missed. Clone of
apollo-dm-waterfall/scripts/apollo_dm_waterfall.py, NOT an in-place edit —
that script backs the LIVE healthcare-demand-pipeline (Indiana/Texas/
Florida) and per CLAUDE.md's own "never modify a live campaign" rule, its
RANK_SYSTEM must not be repointed at a different vertical out from under it.
Same convention as every other per-client copy script in this repo: clone,
don't parameterize.

Ordering (Jude, 2026-08-11): Purple Magic runs FIRST for this vertical
(find_dm_pm.py), this script SECOND as the fallback over PM's misses. PM
writes its own attempt status to col AC, never touching this script's status
column (AB) — so a PM miss (blank email, blank AB) is picked up here
automatically with zero extra wiring, and a PM find (filled email) is
correctly skipped.
...
Per row (has website, no email yet):
  ...
  2. Azure OpenAI GPT-4.1 ranks up to 3 candidates (P1-P3) against a single-
     tier positive gate (Jude, 2026-08-11, no campaign data yet for this
     vertical): Owner/Founder/CEO/President/Managing Director/Managing
     Partner/Principal, or nobody. Same ladder as find_dm_pm.py's OWNER_RE,
     for consistency across both lanes.
...
Usage:
  python3 -W ignore apollo_dm_waterfall_production.py --sheet_url "URL" [options]
...
# No admin/ops rescue tier in this vertical — the ladder below is already a
# single positive-gate tier (own the business, or nothing). ADMIN_RESCUE
# stays defined and permanently False so the shared process_lead()/main()
# logic below (copied from apollo_dm_waterfall.py) doesn't need forking.
ADMIN_RESCUE = False
ADMIN_TITLES = []
ADMIN_RANK_SYSTEM = ""
...
# the page. Must mirror RANK_SYSTEM exactly. Production houses in this pool
# are 5-40 employees per the brief's own filter, so LARGE essentially never
# triggers — kept for the rare mis-sized row.
LARGE_ORG_TITLES = [
    "chief executive officer", "ceo", "president", "founder", "co-founder",
    "owner", "managing partner", "managing director", "principal",
]

RANK_SYSTEM = """You rank employees of a commercial video production company \
by who most likely owns the decision to accept new client/vendor work — this \
is a pitch ("we have brands looking for production, do you have capacity"), \
not a job requisition, so ignore any OPEN ROLE/JOB LOCATION fields if present.

DECISION LADDER (Jude, 2026-08-11 — starting hypothesis, no campaign data
exists yet for this vertical). ONE tier, positive gate only:

Owner / Co-Owner / Founder / Co-Founder / CEO / President / Managing
Director / Managing Partner / Principal / Proprietor. The top of the house —
at the 5-40 employee shops this list is built from, this person is usually
still the one fielding new-business pitches directly.

Nobody else. If none of these titles is on the candidate list, return an
EMPTY LIST — a lesser title (Executive Producer, Head of Production, Line
Producer, Producer, any Manager/Coordinator) spends an AMF credit, burns the
company, and has no measured basis for a reply here. This mirrors the one
finding this repo DOES have data for (healthcare: owner/CEO outperformed
every other title 4.2x, every interested reply came from an owner) — treated
as a reasonable prior for a similarly owner-operated business size band, not
as evidence specific to this vertical.

NEVER pick, for any reason:
- HR of any level, talent/recruiting staff.
- Executive Producer, Head of Production, Line Producer, Producer, Creative
  Director, or any other production-crew title — these run projects, not the
  business, and are not confirmed to have vendor/BD authority.
- Site or regional managers, Office Manager, Studio Manager, Coordinator.
...
def main():
    ...
    ap.add_argument("--col_job", default="N",
                    help="unused for this vertical (no open-role framing) — "
                         "defaults to Revenue (N), which the schema leaves blank")
    ...
    ap.add_argument("--admin_rescue", action="store_true",
                    help="DISABLED for this vertical — no non-owner rescue tier "
                         "exists here (see RANK_SYSTEM). Flag kept only so the "
                         "shared process_lead()/main() logic doesn't need forking; "
    ...
    ADMIN_RESCUE = False  # hardcoded off — see --admin_rescue help text
