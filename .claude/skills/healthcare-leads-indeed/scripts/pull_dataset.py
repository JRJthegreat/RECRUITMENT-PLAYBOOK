"""
Phase 1: Pull Apify Indeed dataset → Google Sheet (Healthcare pipeline)
...
HEADERS = [
    ...
    # Derived columns
    "Role Type",           # AB — Physician / NP / PA (from job title)
    "Indeed URL",          # AC — https://www.indeed.com/viewjob?jk={Job_Id}
]


def classify_role_type(job_title):
    """Classify clinical role from job title: Physician, NP, or PA."""
...
def main():
    parser = argparse.ArgumentParser(description="Pull Apify Indeed dataset → Google Sheet (Healthcare)")
    ...
    print("=== Pull Apify Dataset → Google Sheet (Healthcare) ===\n")
