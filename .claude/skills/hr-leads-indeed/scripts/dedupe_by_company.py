...
Rules:
  ...
  2. Winner = highest seniority tier
     (CHRO/VP > Director > Head > Manager > Lead > Senior > Generalist/Recruiter/Specialist > Coordinator/Junior).
...
def classify_tier(title):
    t = (title or "").lower()
    # C-suite / VP / Chief HR
    if re.search(r"\b(chro|chief\s+human|chief\s+people|vp|vice\s+president)\b", t):
        return 9
    if re.search(r"\bdirector\b", t):
        return 8
    if re.search(r"\bhead\s+of\b", t):
        return 7
    ...
    if re.search(r"\b(senior|snr|sr\.?)\b", t):
        return 4
    if re.search(r"\b(generalist|recruiter|specialist|partner|consultant|advisor)\b", t):
        return 3
    if re.search(r"\b(coordinator|administrator|assistant|associate)\b", t):
        return 2
    if re.search(r"\b(junior|jnr|jr\.?|intern|trainee|apprentice|entry)\b", t):
        return 1
    return 3  # default: mid-level IC


TIER_LABEL = {
    9: "CHRO/VP", 8: "Director", 7: "Head of", 6: "Manager",
    5: "Lead", 4: "Senior", 3: "IC/Specialist", 2: "Coordinator", 1: "Junior",
}


# --- Company name normalization (US legal suffixes) ---
...
def main():
    parser = argparse.ArgumentParser(description="Dedup HR leads by company")
    ...
    for key, items in groups.items():
        ...
        items.sort(key=lambda x: x["tier"], reverse=True)
        winner = items[0]
        winner["action"] = "KEEP"
