"""
Phase 1: Orchestrate Apify Indeed scrapes → Google Sheet.
...
last N days, no duplicate Job_Ids). Streams rows into the sheet in batches
...
Usage:
  ...
  --cities "A,B,..."     comma-separated override
...
# Fixed keyword list — split "Structural/Bridge" into two entries so each
# actor run takes ONE title (per user requirement: never all at once).
DEFAULT_KEYWORDS = [
    "Civil Engineer",
    "Highways Engineer",
    "Drainage Engineer",
...
DEFAULT_CITIES = [
    ...
    "Glasgow", "Edinburgh", "Bristol", "Newcastle", "Sheffield",
    "Cardiff", "Nottingham", "Southampton", "Leicester", "Belfast",
    "Aberdeen", "Cambridge", "Oxford",
]
...
def run_actor(keyword, city, days, limit, timeout=180):
    ...
    try:
        ...
        print(f"  [!] {keyword} @ {city}: {type(e).__name__}: {e}")
        return []

    if resp.status_code not in (200, 201):
        print(f"  [!] {keyword} @ {city}: HTTP {resp.status_code} — {resp.text[:120]}")
        return []

    try:
        return resp.json() or []
    except ValueError:
        print(f"  [!] {keyword} @ {city}: invalid JSON response")
        return []
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Apify Indeed (keyword × city grid) → Google Sheet")
    ...
    parser.add_argument("--cities", default="", help="Comma-separated override")
    ...
    print("=== Apify Indeed Scrape Orchestrator ===")
    ...
    # Snapshot the existing set; threads read-only. Main thread updates
    # `seen` (existing + already-written in this session) as rows stream in.
    ...
    def work(kw, ct):
        items = run_actor(kw, ct, args.days, limit)
        rows, stats = filter_items(items, existing_job_ids, cutoff_date)
        return kw, ct, items, rows, stats

    print(f"Launching {len(combos)} runs with {args.workers} workers...\n")

    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(work, kw, ct): (kw, ct) for kw, ct in combos}
        done_count = 0
        for fut in as_completed(futures):
            done_count += 1
            kw, ct = futures[fut]
            try:
                kw_r, ct_r, items, rows, stats = fut.result()
            except Exception as e:
                print(f"  [{done_count}/{len(combos)}] {kw} @ {ct}: EXC {e}")
                totals["runs_fail"] += 1
                continue

            if not items and stats["total"] == 0:
                # Count as fail only if run_actor returned empty due to error;
                # a legitimately-empty scrape still counts as ok.
                totals["runs_fail"] += 1
            else:
                totals["runs_ok"] += 1
            ...
            # Flush whenever we've buffered a full batch
            ...
            print(f"  [{done_count}/{len(combos)}] {kw:22s} @ {ct:14s}  "
    ...
    print(f"  Skipped >500 emp:   {totals['too_big']:,}")
