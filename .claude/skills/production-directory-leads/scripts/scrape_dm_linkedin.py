"""
Phase 0 (icebreaker research) - pull the decision maker's LinkedIn profile.

Clone of personalized-icebreakers/scripts/scrape_linkedin.py, adapted to the
master sheet's 29-col schema (V:LinkedIn URL). Purely mechanical (Apify actor
call) — no LLM anywhere in this script, same as the original.

Actor: dev_fusion~Linkedin-Profile-Scraper ($3 per 1,000 profiles).

Writes a compacted JSON blob to --col_out. Batch-of-10 writes. Resume-safe:
skips rows where --col_out is filled. Profiles get blocked intermittently;
rows that come back empty are left blank and recovered by re-running.

Run:
  python3 -W ignore scrape_dm_linkedin.py --sheet_url "URL" [--tab Leads] \
    [--limit N] [--preview N]
"""
import os
import re
...
COL_LINKEDIN = 21  # V, 0-indexed
COL_OUT = 29       # AD — first column past the 29-col base schema (AB=status, AC=PM status)
...
def compact(item):
    ...
    for e in (item.get("experiences") or [])[:6]:
        exps.append({"title": clean(e.get("title"), 120),
...
def main():
    ...
    service = get_service()
    last = col_letter(max(COL_LINKEDIN, COL_OUT))
    rows = service.spreadsheets().values().get(
        spreadsheetId=sheet_id, range=f"'{tab}'!A:{last}").execute().get("values", [])[1:]

    pending = []
    for i, row in enumerate(rows):
        li = row[COL_LINKEDIN].strip() if len(row) > COL_LINKEDIN else ""
        out = row[COL_OUT].strip() if len(row) > COL_OUT else ""
        if not li or out:
            continue
    ...
    print(f"=== DM LinkedIn Profile Scrape ===\nActor: {ACTOR_ID}")
        ...
        if cur < COL_OUT + 1:
            service.spreadsheets().batchUpdate(
                spreadsheetId=sheet_id, body={"requests": [{"appendDimension": {
                    ...
                    "dimension": "COLUMNS", "length": COL_OUT + 1 - cur}}]}).execute()
        service.spreadsheets().values().update(
            spreadsheetId=sheet_id, range=f"'{tab}'!{col_letter(COL_OUT)}1",
        ...
        for fut in as_completed(futs):
            ...
            for it in items:
                ...
                if args.preview:
                    ...
                    print(f"  company : {data['company']} ({data['company_size']})")
            ...
            if not args.preview and len(updates) >= WRITE_BATCH:
                service.spreadsheets().values().batchUpdate(
                    spreadsheetId=sheet_id, body={"valueInputOption": "RAW", "data": [
                        {"range": f"'{tab}'!{col_letter(COL_OUT)}{r}", "values": [[v]]}
