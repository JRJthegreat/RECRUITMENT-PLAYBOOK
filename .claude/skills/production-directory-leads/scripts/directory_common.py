"""
Shared helpers for the production-directory-leads skill: config loading,
SQLite store, and domain normalization.

Not a pipeline phase — imported by the phase scripts in this directory.

Clone of production-house-leads/production_common.py with a directory-shaped
store: rows are keyed by ProductionHUB profile_id (not Maps place_id) and
carry the directory's own fields (description, member_since, linkedin_url,
vimeo_url) instead of ratings/reviews. Same nppes-style model otherwise:
scrape everything into data/directory.db, classify in place, export campaign
batches with exported_at/batch_id stamps so a company is never worked twice.
"""
import json
import os
import sqlite3
...
SOCIAL_HOSTS = {
    ...
    "business.site", "google.com", "yelp.com", "imdb.com",
}
