"""
Phase 3 (demand side): Find DMs and/or emails via AnyMail Finder for the
29-col healthcare practice sheet (company=K, DM=T/U/V, email=W).

This is the DEMAND-side enricher. Do NOT use enrich_emails.py in this skill —
that one is the repurposed staffing-agency version (company=C, email=Q).

Two modes processed in one pass:

  Mode A (person lookup) — row has DM Name (col T) but no Email (col W):
    POST /v5.1/find-email/person  with full_name + domain  → writes email to W.
  ...
  Mode B (decision-maker lookup) — row has no DM Name but col L has a domain:
    ...
    Per Jude: fall back to OWNER and HR only. Tier → AMF category (primary, fallback):
      practice_manager → ceo → hr   (small practice: owner is the buyer)
      hr               → hr  → ceo  (51-200: HR owns recruiting)
      owner            → ceo → hr
    Writes name → T, title → U, linkedin → V, email → W.

AMF auth: "Authorization: {API_KEY}" (no "Bearer").
Accepts email_status == valid only. Risky emails are rejected. "not_found" is written back to W so
...
from find_dm_demand import determine_target
...
# Per Jude: AMF /decision-maker fallback uses OWNER (ceo) and HR only.
TIER_TO_CATEGORIES = {
    "practice_manager": ("ceo", "hr"),
    "hr":               ("hr", "ceo"),
    "owner":            ("ceo", "hr"),
}
...
def _is_same_level_conflict(job_title, dm_title):
    """No-op on the demand side. The role being hired is always clinical
    (NP/PA/physician), never HR/admin, so AMF can't return 'the person being
    hired' as the DM — the HR-pipeline conflict check doesn't apply."""
    return False
...
def main():
    ...
    # Dedup so we pay AMF once per unique person/domain and fan the result out
    # to every sibling row (multi-location practices repeat the same domain).
    person_by_key, dm_by_key = {}, {}
    for i, row in enumerate(data_rows):
        ...
        if dm_name and not args.dm_only:
            key = (dm_name.strip().lower(), domain or company_name.strip().lower())
            if key in person_by_key:
                person_by_key[key]["sibling_rows"].append(sr)
                continue
            person_by_key[key] = {
                "sheet_row": sr,
                "dm_name": dm_name,
            ...
            if domain in dm_by_key:
                dm_by_key[domain]["sibling_rows"].append(sr)
                continue
            dm_by_key[domain] = {
                "sheet_row": sr,
                "company_name": company_name,
    ...
    person_leads = list(person_by_key.values())
    dm_leads = list(dm_by_key.values())
        ...
        for b in range(batches):
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
                ...
                for fut in as_completed(futures):
                    r = fut.result()
                    sibs = r.get("sibling_rows", [r["sheet_row"]])
                    val = r["email"] if r.get("email") else "not_found"
                    fan = f" (+{len(sibs) - 1})" if len(sibs) > 1 else ""
                    if r.get("email"):
                        print(f"    row {r['sheet_row']}: {r['dm_name']} -> {r['email']}{fan}")
                        total_email_found += 1
                    else:
                        print(f"    row {r['sheet_row']}: {r['dm_name']} -> not_found ({r['status']}){fan}")
                        total_email_miss += 1
                    for sr in sibs:
                        updates.append({"range": f"'{TAB_NAME}'!{col_letter(COL_EMAIL)}{sr}", "values": [[val]]})
        ...
        for b in range(batches):
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
                ...
                for fut in as_completed(futures):
                    ...
                    if name:
                        print(f"    row {r['sheet_row']} {tag}: {name} <{email or 'no-email'}> -- {title} -- {r['company_name']}{fan}")
                        if email:
                            total_dm_found += 1
                        else:
                            total_dm_miss += 1
                        for sr in sibs:
                            ...
                            updates.append({"range": f"'{TAB_NAME}'!{col_letter(COL_EMAIL)}{sr}", "values": [[email or "not_found"]]})
                    else:
                        print(f"    row {r['sheet_row']} {tag}: no DM found ({r.get('status', '?')}) -- {r['company_name']}{fan}")
                        total_dm_miss += 1
                        for sr in sibs:
